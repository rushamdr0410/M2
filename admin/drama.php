...
$drama_genre_id = 18; // TMDb genre ID for Action
...
$movies_url = "$tmdb_base_url/discover/movie?api_key=$tmdb_api_key&with_genres=$drama_genre_id&sort_by=popularity.desc&page=$current_page";
$movies_data = fetch_tmdb_data($movies_url);
$drama_movies = $movies_data['results'] ?? [];
$total_pages = min($movies_data['total_pages'] ?? 10, 10); // Limit to max 10 pages

// Fetch drama TV shows from TMDb with pagination
$tv_url = "$tmdb_base_url/discover/tv?api_key=$tmdb_api_key&with_genres=$drama_genre_id&sort_by=popularity.desc&page=$current_page";
$tv_data = fetch_tmdb_data($tv_url);
$drama_tvshows = $tv_data['results'] ?? [];
...
<body>
  ...
  <h1 class="watchlist-header">Dramatic Movies & TV-Shows</h1>
        
  <!-- TMDb Action Movies -->
  <?php if (!empty($drama_movies)): ?>
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular drama Movies</h2>
    ...
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular drama TV Shows</h2>
    <div class="watchlist-movies">
      <?php foreach ($drama_tvshows as $tvshow): ?>
        <div class="watchlist-movie">
          <div class="img">
