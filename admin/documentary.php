...
$documentary_genre_id = 99; // TMDb genre ID for Action
...
$movies_url = "$tmdb_base_url/discover/movie?api_key=$tmdb_api_key&with_genres=$documentary_genre_id&sort_by=popularity.desc&page=$current_page";
$movies_data = fetch_tmdb_data($movies_url);
$documentary_movies = $movies_data['results'] ?? [];
$total_pages = min($movies_data['total_pages'] ?? 10, 10); // Limit to max 10 pages

// Fetch documentary TV shows from TMDb with pagination
$tv_url = "$tmdb_base_url/discover/tv?api_key=$tmdb_api_key&with_genres=$documentary_genre_id&sort_by=popularity.desc&page=$current_page";
$tv_data = fetch_tmdb_data($tv_url);
$documentary_tvshows = $tv_data['results'] ?? [];
...
<body>
  ...
  <h1 class="watchlist-header">Documentaries</h1>
        
  <!-- TMDb Action Movies -->
  <?php if (!empty($documentary_movies)): ?>
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular documentary Movies</h2>
    <div class="watchlist-movies">
      <?php foreach ($documentary_movies as $movie): ?>
    ...
    <?php if (!empty($documetary_tvshows)): ?>
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular documetary TV Shows</h2>
    <div class="watchlist-movies">
      <?php foreach ($documetary_tvshows as $tvshow): ?>
        <div class="watchlist-movie">
          <div class="img">
