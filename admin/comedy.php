...
$comedy_genre_id = 35; // TMDb genre ID for Action
...
$movies_url = "$tmdb_base_url/discover/movie?api_key=$tmdb_api_key&with_genres=$comedy_genre_id&sort_by=popularity.desc";
$movies_data = fetch_tmdb_data($movies_url);
$comedy_movies = $movies_data['results'] ?? [];

// Fetch action TV shows from TMDb
$tv_url = "$tmdb_base_url/discover/tv?api_key=$tmdb_api_key&with_genres=$comedy_genre_id&sort_by=popularity.desc";
$tv_data = fetch_tmdb_data($tv_url);
$comedy_tvshows = $tv_data['results'] ?? [];

// Query to fetch only Action movies (genreid = 1)
$query = "SELECT * FROM moviedetails WHERE genreid = '9'";
...
<body>
  ...
  <h1 class="watchlist-header">Comedy Movies & TV-Shows</h1>
        
  <!-- TMDb Action Movies -->
  <?php if (!empty($comedy_movies)): ?>
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular comedy Movies</h2>
    ...
    <h2 style="color: #61DAFB; margin: 40px 0 20px; text-align: center;">Popular comedy TV Shows</h2>
    <div class="watchlist-movies">
      <?php foreach ($comedy_tvshows as $tvshow): ?>
        <div class="watchlist-movie">
          <div class="img">
